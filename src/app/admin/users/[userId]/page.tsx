...
import { 
  ...
  Settings2, PlusCircle, MinusCircle, Equal, IndianRupee
} from "lucide-react";
...
export default function UserDetailPage() {
  ...
  const handleBalanceUpdate = async () => {
    ...
    if (isNaN(amount) || amount < 0) {
      toast({ variant: "destructive", title: "Invalid Amount", description: "Please enter a numeric value." });
      return;
    }
  ...
  if (loading && !user) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#F8FAFC]">
        ...
        <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Syncing Node Data...</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <h1 className="text-xl font-black">Node Not Found</h1>
          <Button onClick={() => router.push('/admin')} className="mt-4">Back to Terminal</Button>
        </div>
      </div>
    );
  }
              ...
              <div className="space-y-4">
                <div className="bg-slate-900 rounded-[2rem] p-6 text-white relative overflow-hidden group">
                  ...
                  <Dialog open={isBalanceDialogOpen} onOpenChange={setIsBalanceDialogOpen}>
                    <DialogTrigger asChild>
                      <Button className="w-full bg-white text-slate-900 hover:bg-slate-100 font-black uppercase text-[10px] tracking-widest h-11 rounded-xl shadow-lg">
                        MANAGE ASSETS
                      </Button>
                    </DialogTrigger>
                    <DialogContent className="max-w-md rounded-[2rem] p-8 border-0">
                      <DialogHeader>
                        ...
                        <DialogDescription className="text-[10px] font-bold uppercase text-slate-400">Manage liquidity for node {user.id.slice(0, 8).toUpperCase()}</DialogDescription>
                      </DialogHeader>
                      <div className="py-6 space-y-6">
                        ...
                        <Input type="number" placeholder="Enter amount..." value={adjustAmount} onChange={e => setAdjustAmount(e.target.value)} className="h-14 bg-slate-50 border-0 rounded-2xl text-lg font-black text-center"/>
                      </div>
                      <DialogFooter>
                        <Button onClick={handleBalanceUpdate} disabled={isUpdatingBalance} className="w-full h-12 bg-slate-900 hover:bg-black font-black uppercase text-[10px] tracking-widest rounded-xl">
                          {isUpdatingBalance && <RefreshCw className="animate-spin mr-2" size={14}/>} EXECUTE PROTOCOL
                        </Button>
                      </DialogFooter>
                    </DialogContent>
                  </Dialog>
                  <Wallet size={100} className="absolute -bottom-6 -right-6 opacity-10 rotate-12" />
                </div>
                ...
                <div className="space-y-2">
                   {[
                     ...
                     { label: "Joined", value: new Date(user.created_at).toLocaleDateString(), icon: Activity },
                   ].map((item, i) => (
          ...
          <div className="flex-1 space-y-6">
             <Tabs defaultValue="linked" className="w-full">
                <TabsList className="w-full bg-white h-16 p-2 rounded-[2rem] border border-slate-100 shadow-sm">
                   ...
                   <TabsTrigger value="history" className="flex-1 rounded-2xl text-[10px] font-black uppercase">Activity Logs ({orders.length})</TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="history" className="mt-6 space-y-4">
                   <div className="bg-white p-4 rounded-[1.8rem] border border-slate-100 shadow-sm flex gap-4">
                      <div className="relative flex-1">
                         ...
                         <Input placeholder="Search UTR, ID, or Amount..." value={logSearch} onChange={e => setLogSearch(e.target.value)} className="h-12 pl-10 bg-slate-50 border-0 rounded-xl text-[11px] font-bold uppercase"/>
                      </div>
                   </div>
                   <div className="grid gap-3">
                      {filteredLogs.map((o, i) => (
                        <Card key={i} className="border-0 p-5 rounded-[1.8rem] bg-white shadow-sm flex items-center justify-between">
                           <div className="flex items-center gap-4">
                              <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center", o.status === 'success' ? "bg-emerald-50 text-emerald-500" : "bg-amber-50 text-amber-500")}>
                                 <History size={18} />
                              </div>
                              <div>
                                 <span className="text-[12px] font-black uppercase tracking-tight">{o.id}</span>
                                 <p className="text-[8px] font-bold text-slate-400 uppercase mt-0.5">{new Date(o.created_at).toLocaleString()} {o.utr ? `• UTR: ${o.utr}` : ''}</p>
                              </div>
                           </div>
                           <div className="text-right">
                              <p className="text-lg font-black text-slate-900">₹{o.amount.toLocaleString()}</p>
                              <Badge variant="outline" className="text-[7px] font-black border-slate-100 h-5 px-2 uppercase">{o.status}</Badge>
                           </div>
                        </Card>
                      ))}
                   </div>
